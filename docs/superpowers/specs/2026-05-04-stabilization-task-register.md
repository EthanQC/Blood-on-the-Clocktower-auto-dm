...
| A-020 | Self-check before final | Allowed non-manual | completed | yes | none | User request | Keep final output with delivery summary | Required `rg` checks and `git diff --check` produced no findings; status shows only preserved untracked user files. |
...
| C-006 | Accepted Layer 4 AutoDM in-game trigger | Post-provider manual QA | ready-on-gemini | yes | RAG container mount needs correction before accepted RAG closure | Plan Task 6 Step 7 | Start after Layer 1 or explicit AutoDM-only decision | Must observe real provider output tied to active room. |
